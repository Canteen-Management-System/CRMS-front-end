...
function ReportList() {
  ...
  const filteredUser = retrievedTasks.filter((employee) => {
    return employee.assign_to === parseInt(decision);
  });
  ...
  const handleChangeanalysis3 = (e) => {
    ...
    console.log(x2);
    const yy = [];
    services.map((ser) => yy.push(filterServiceType(ser)));
    sety2((y2 = yy));
    setanalysis1(false);
    setanalysis2(false);
    setanalysis3(true);
  ...
  const handleChangeanalysis4 = (e) => {
    ...
    console.log(x3);
    const yyy = [];
    Categories.map((ser) => yyy.push(filterCategory(ser)));
    sety3((y3 = yyy));
    setanalysis1(false);
    setanalysis2(false);
    setanalysis3(false);
  ...
  return (
    <>
      <div className="flex flex-col items-center justify-center w-full px-3 py-3 rounded-md shadow-md h-1/2">
        <legend className="md:text-3xl pb-8 text-[#F2F2F2] py-5">
          Reports
        </legend>
        <div className="w-1/2 mb-6 text-black ">
          <form>
            <label className="w-1/2 pr-4 text-lg text-white "> Staff </label>
            <select value={decision} onChange={handleChange}>
              <option className="pr-4 text-lg text-white " value=" " />
            ...
            <label className="w-1/2 pr-4 ml-4 text-lg text-white "> Date</label>
            <input type="date" onChange={handleDateChange} />
          </form>
        </div>
        <table className="w-1/2 text-white border border-collapse border-separate border-slate-400">
          <thead className="bg-gray">
            <tr>
              <td className="px-4 bg-gray-600 border border-slate-300">no.</td>
              <td className="px-4 bg-gray-600 border border-slate-300 ">
                Report Name{" "}
              </td>
              <td className="px-4 bg-gray-600 border border-slate-300">
                download
          ...
          <tbody>
            <tr>
              <td className="px-4 border border-slate-300"> 1.</td>
            ...
            <tr>
              <td className="px-4 border border-slate-300"> 2.</td>
              <td className="px-4 border border-slate-300">Filter By name</td>
              <td className="px-4 border border-slate-300">
                <button onClick={handleExportReport2}>Excel</button>
              </td>
            </tr>
            <tr>
              <td className="px-4 border border-slate-300"> 3.</td>
              <td className="px-4 border border-slate-300">
                Resolved Complain
              </td>
              <td className="px-4 border border-slate-300">
            ...
            <tr>
              <td className="px-4 border border-slate-300"> 4.</td>
              <td className="px-4 border border-slate-300">Pending Complain</td>
              <td className="px-4 border border-slate-300">
      ...
      <div className="flex flex-col items-center justify-center">
        {analysis1 ? (
          <BarPlot
            x={x}
            y={y}
            title={` Total Tasks as per the status for ${Datevalue}`}
          />
        ) : analysis2 ? (
          ...
          <ParPlot
            x={x2}
            y={y2}
            title={"Total Number of Tasks as per the service type"}
          />
        ) : analysis4 ? (
